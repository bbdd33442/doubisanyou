...
public class BoxMainActivity extends TabActivity {
	...
	private Class[] classes = new Class[] { TeaKnowledgeActivity.class,
			TeaChatActivity.class, TeaSayActivity.class, ManagerActivity.class };
	// tab选中时显示的图片
		...
		tabHost.setOnTabChangedListener(new TabHost.OnTabChangeListener() {
			...
			public void onTabChanged(String tabId) {
				for (int i = 0; i < tabwidget.getChildCount(); i++) {
					LinearLayout tabIndicator = (LinearLayout) tabwidget
							.getChildAt(i);
					ImageView iv = (ImageView) tabIndicator
							.findViewById(R.id.item_TabImg);
					TextView tv = (TextView) tabIndicator
							.findViewById(R.id.item_TabName);

					/*
					 * ImageView abLayout =
					 * (ImageView)tabIndicator.findViewById(R.id.tabimg_back_);
					 */
	...
	public void addNewBotTab(int index, int resId, Class<?> cls, String tag) {
		...
		TabSpec tabSpec = tabHost.newTabSpec(tag).setIndicator(tabIndicator)
