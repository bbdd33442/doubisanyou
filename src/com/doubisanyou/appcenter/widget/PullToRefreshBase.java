...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	private boolean isDownRefreshEnabled = true;
	
	private boolean isUpRefreshEnabled = true;
	...
	public final void setUpRefreshEnabled(boolean enable) {
		this.isUpRefreshEnabled = enable;
	}
	public final void setDownRefreshEnabled(boolean enable) {
		this.isDownRefreshEnabled = enable;
	}
				...
				if (isReadyForPull()) {
					...
					if (yDiff > touchSlop && yDiff > xDiff) {
						if ((mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) && dy >= 0.0001f
								&& isReadyForPullDown() &&isDownRefreshEnabled) {
							lastMotionY = y;
							isBeingDragged = true;
							if (mode == MODE_BOTH) {
								...
								&& isReadyForPullUp() && isUpRefreshEnabled) {
							lastMotionY = y;
							isBeingDragged = true;
							if (mode == MODE_BOTH) {
